...
> **Archived rejected production path (2026-08-09).** Epic 1 completed this validation harness with `reject`. It remains reproducible historical evidence and a source of provider-neutral contracts, but it does not authorize production Codex App Server work. The current MVP architecture uses Ollama, LM Studio, and MLX as first-class local adapters and OpenRouter as the optional external adapter; see `_bmad-output/planning-artifacts/architecture/architecture-ProjectOS-2026-07-31/ARCHITECTURE-SPINE.md`.
