...
Story 1.7 writes atomic `conversation-ownership-summary.json` records in separate `*-conversation-ownership/` directories and follows `conversation-ownership-validation-run.schema.json`. It retains only run/correlation IDs, outcome, SHA-256 digests, fixed structural counts, check names, and stop conditions. It rejects conversation content, local IDs, bindings, session handles, adapter metadata, credentials, authentication/runtime/cache state, diagnostics, payloads, and paths. The deterministic proof is offline and fake-backed; it is not evidence of live provider resume or preventive containment.

Story 1.8 writes atomic `provider-cleanup-summary.json` records in separate `*-provider-cleanup/` directories and follows `provider-cleanup-validation-run.schema.json`. It retains only aggregate created/terminal/pending accounting, structural checks, and the two mandatory stop conditions. It rejects handles, profile/context values, content, local IDs, credentials, identity, paths, and raw payloads. The durable outbox and fake filesystem are private temporary validation state; neither is portable Project state or shareable evidence. The conclusion is always `reject` until an approved manifest-pinned live Codex list/delete contract and preventive containment proof exist.

Story 1.9 reads an explicit prerequisite root containing `gate-decision-sources.json` and one bounded source record per mandatory gate. The loader rejects symlinks, unsafe names/text, unexpected keys, missing/duplicate gates, source-digest/identity mismatches, and any caller-supplied manifest that differs from `approved-current-gate-manifest.json`. Only the loader can create a bundle accepted by the reducer and recorder. `gate-decision-summary.json` retains the aggregate decision, safe failed-gate codes, source-manifest digest, and reproduction command; it retains no source content, identity, session/binding, prompt/result, credential, or local-path material. The summary is flushed in a private staging directory before rename. A failed post-rename directory sync leaves the complete summary visible and reports durability as `unknown`; no post-publication cleanup/truncation is permitted.
